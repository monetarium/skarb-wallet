...
// TestSendRawTransactionMixedCoinTypeGuard pins the MED-4 fix from the
// 2026-05-04 review: sendrawtransaction must refuse a hex tx whose outputs
// are not all the same coin type, before dispatching to TxPaysHighFees /
// TxPaysHighFeesSKA (which only inspect outputs[0]) and before broadcasting.
//
// The guard is the small loop at the top of sendRawTransaction in methods.go.
// This test exercises the equivalent logic against synthetic msgtx values so
// a refactor that drops the loop is caught without standing up a wallet
// fixture (no NetworkBackend / loaded-wallet harness in this package).
func TestSendRawTransactionMixedCoinTypeGuard(t *testing.T) {
	...
	tests := []struct {
		...
		outs     []*wire.TxOut
		wantFail bool
		wantBadIdx int // 1-based index in the failure message
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Mirror the production guard in sendRawTransaction.
			var (
				gotFail   bool
				gotBadIdx int
			)
