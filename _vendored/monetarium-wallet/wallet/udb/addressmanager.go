...
func (m *Manager) RetrieveEmissionKey(ns walletdb.ReadBucket, keyName string) (*secp256k1.PrivateKey, error) {
	...
	// zero so DB corruption or an encrypt-side bug surfaces here
	// instead of returning a silently-invalid signing key. Mirrors the
	// validation already enforced on the import path in the RPC layer.
	if len(privKeyBytes) != 32 {
		for i := range privKeyBytes {
			privKeyBytes[i] = 0
		}
		return nil, errors.E(errors.IO, errors.Errorf(
			"decrypted emission key has invalid length %d (expected 32)", len(privKeyBytes)))
	}
	...
	if privateKey.Key.IsZero() {
		...
		return nil, errors.E(errors.IO, "decrypted emission key is zero or out of range")
	}
...
// emissionAuthLogKey serializes the (CoinType, Nonce) pair used to key the
// emissionauthlog bucket. CoinType is one byte; nonce is big-endian so a
// future range-walk preserves natural ordering.
func emissionAuthLogKey(coinType uint8, nonce uint64) []byte {
	key := make([]byte, 9)
	key[0] = coinType
...
// emissionAuthLogValue serializes a 32-byte tx hash and its signing-time
// unix timestamp.
func emissionAuthLogValue(txHash [32]byte, timestamp int64) []byte {
	val := make([]byte, 40)
	copy(val[:32], txHash[:])
	binary.BigEndian.PutUint64(val[32:], uint64(timestamp))
	return val
}

// LookupEmissionAuthRecord returns the prior tx hash and timestamp recorded
// for a (CoinType, Nonce) pair, or (zero, false, nil) if no record exists.
// The bucket is created lazily on first write, so a missing bucket is not
// an error.
func (m *Manager) LookupEmissionAuthRecord(ns walletdb.ReadBucket, coinType uint8, nonce uint64) (txHash [32]byte, timestamp int64, exists bool, err error) {
	bucket := ns.NestedReadBucket(emissionAuthLogBucketName)
	if bucket == nil {
		return txHash, 0, false, nil
	}
	val := bucket.Get(emissionAuthLogKey(coinType, nonce))
	if val == nil {
		return txHash, 0, false, nil
	}
	if len(val) != 40 {
		return txHash, 0, false, errors.E(errors.IO,
			errors.Errorf("emissionauthlog record for (cointype=%d, nonce=%d) has invalid length %d (expected 40)",
				coinType, nonce, len(val)))
	}
	copy(txHash[:], val[:32])
	timestamp = int64(binary.BigEndian.Uint64(val[32:]))
	return txHash, timestamp, true, nil
}

// StoreEmissionAuthRecord persists a (CoinType, Nonce) → (TxHash, Timestamp)
// record. Idempotent: overwrites any existing record at the same key. The
// caller is expected to consult LookupEmissionAuthRecord first and refuse
// the call when a record already exists, unless the user has explicitly
// passed forcenonce=true.
func (m *Manager) StoreEmissionAuthRecord(ns walletdb.ReadWriteBucket, coinType uint8, nonce uint64, txHash [32]byte, timestamp int64) error {
	bucket, err := ns.CreateBucketIfNotExists(emissionAuthLogBucketName)
	if err != nil {
		return err
	}
	return bucket.Put(emissionAuthLogKey(coinType, nonce), emissionAuthLogValue(txHash, timestamp))
}

// DeleteEmissionAuthRecord removes the record at (CoinType, Nonce). Used by
// callers that observe the prior emission tx confirm or expire on chain so
// the local guard does not become stale.
func (m *Manager) DeleteEmissionAuthRecord(ns walletdb.ReadWriteBucket, coinType uint8, nonce uint64) error {
	bucket := ns.NestedReadWriteBucket(emissionAuthLogBucketName)
	if bucket == nil {
		return nil
	}
	return bucket.Delete(emissionAuthLogKey(coinType, nonce))
}
