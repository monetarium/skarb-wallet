...
import (
	...
	pb "decred.org/monwallet/v5/rpc/walletrpc"
...
```bash
$ protoc -I/path/to/monwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/monwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by monwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.monwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\monwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by monwallet).
            Environment.SetEnvironmentVariable("GRPC_SSL_CIPHER_SUITES", "HIGH+ECDSA");

            var walletAppData = Portability.LocalAppData(Environment.OSVersion.Platform, "Monwallet");
...
if (os.platform() == 'win32') {
  certPath = path.join(process.env.LOCALAPPDATA, 'Monwallet', 'rpc.cert');
} else if (os.platform() == 'darwin') {
  certPath = path.join(process.env.HOME, 'Library', 'Application Support',
    'Monwallet', 'rpc.cert');
} else {
  certPath = path.join(process.env.HOME, '.monwallet', 'rpc.cert');
}
...
```bash
$ protoc -I /path/to/decred/monwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    if platform.system() == 'Windows':
        cert_file_path = os.path.join(os.environ['LOCALAPPDATA'], "Monwallet", "rpc.cert")
    elif platform.system() == 'Darwin':
        cert_file_path = os.path.join(os.environ['HOME'], 'Library', 'Application Support',
                                      'Monwallet', 'rpc.cert')
    else:
        cert_file_path = os.path.join(os.environ['HOME'], '.monwallet', 'rpc.cert')
