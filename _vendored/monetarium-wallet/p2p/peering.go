...
type RemotePeer struct {
	...
	// recentlyConsumedTxs is a fixed-size LRU of tx hashes that arrived as a
	// valid response to a getdata and got consumed off requestedTxs.  When
	// monetarium-node sends a duplicate MsgTx (SKAn txs appear to be pushed
	// via an additional relay path on top of the standard inv→getdata→tx
	// exchange — see Skarb-wallet investigation Aug 2026), the second copy
	// would otherwise be rejected as "received unrequested tx" and the peer
	// disconnected.  Treating duplicates within this LRU window as a no-op
	// keeps the connection alive without weakening the protocol-violation
	// check for genuinely unrequested txs.
	recentlyConsumedTxs lru.Cache[chainhash.Hash]
...
func handshake(ctx context.Context, lp *LocalPeer, id uint64, na *addrmgr.NetAddress, c net.Conn) (*RemotePeer, error) {
	...
	rp := &RemotePeer{
		...
		recentlyConsumedTxs: lru.NewCache[chainhash.Hash](invLRUSize),
...
func (rp *RemotePeer) receivedTx(ctx context.Context, msg *wire.MsgTx) {
	...
	rp.requestedTxsMu.Unlock()
	if !ok {
		// monetarium-node has been observed to send the same MsgTx twice for
		// SKAn transactions — once as a normal getdata response and a second
		// time via an extra relay path — which trips the strict protocol
		// check upstream dcrwallet inherits from dcrd. Treat duplicates
		// within the recently-consumed LRU window as a no-op so the SPV
		// connection stays up; everything outside that window is still a
		// genuine protocol violation and earns a disconnect.
		if rp.recentlyConsumedTxs.Contains(txHash) {
			log.Debugf("remotepeer(%v).receivedTx(%v): duplicate after consume, ignoring",
				rp.raddr, &txHash)
			return
		}
	...
	rp.recentlyConsumedTxs.Add(txHash)
	select {
	case <-ctx.Done():
	case c <- msg:
	}
